...
if ($_FILES['file3']['error'] === UPLOAD_ERR_OK) {
    $deathName = $_FILES['file3']['name'];
    $deathPath = $folderName . "/" . $deathName;
    move_uploaded_file($_FILES['file3']['tmp_name'], $deathPath);
}

$sql = "INSERT INTO transfer_ownership (date, old_name, account_number, new_name, contact_number, rep_id, waiver, death, status, type, reason)
        VALUES ('$date', '$oldAccountName', '$accountNumber', '$newAccountName', '$contactNumber', '$repIdName', '$waiverName', '$deathName', 'PENDING', 'REPRESENTATIVE', '$reason')";
